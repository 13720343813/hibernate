...
public class MyDate {
    ...
    public String timeFormt = "HH:mm:ss";
    public final String UTCSDATETIMEFORMAT = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'";

    public String getTime() {
        Date date = new Date();
        SimpleDateFormat buldFormat = new SimpleDateFormat(timeFormt);
        String build = buldFormat.format(date);
//		System.out.println("build = " + build);
     ...
     * @return
     */
    public String millisecondToDateTime(Long milli) {        //毫秒
        SimpleDateFormat df = new SimpleDateFormat(dateTimeFormat);
    ...
    /**
     * 获取 yyyy-MM-dd'T'HH:mm:ss.SSS'Z' 格式当前时间
     *
     * @return
     */
    public String currentDateTimetSssz() {
        Date date = new Date();
        SimpleDateFormat format = new SimpleDateFormat(UTCSDATETIMEFORMAT);
        String dateTime = format.format(date);
        return dateTime;
    }

    /**
     * yyyy-MM-dd'T'HH:mm:ss.SSS'Z' 格式 当前时间 减去天数
     *
     * @param dateTime yyyy-MM-dd'T'HH:mm:ss.SSS'Z' 格式时间
     * @param days     减去天数
     * @return
     */
    public String dateTimetSsszMinusDays(String dateTime, int days) {
        return getCurrentDateTimeMinusDays(UTCSDATETIMEFORMAT, dateTime, days);
    }
     ...
     * @return
     */
    public String getCurrentDateTimeMinusDays(String format, String dateTime, int days) {
        SimpleDateFormat simpleformat = new SimpleDateFormat(format);
        Long millisecond = parseDateFormatToTime(dateTime, format);

        Long newmillisecond = (millisecond) - (days * 24 * 60 * 60 * 1000);
        Date newDate = new Date(newmillisecond);

        return simpleformat.format(newDate);
    }

    /**
     * 减去对应的分钟数获取传入时间的格式字符串
     *
     * @param format       传入时间格式
     * @param dateTime     时间
     * @param minusMinutes 减去的分钟数
     * @return
     */
    public String getCurrentDateTimeMinusMinutes(String format, String dateTime, int minusMinutes) {
        SimpleDateFormat simpleformat = new SimpleDateFormat(format);
        Long millisecond = 0L;
        try {
        ...
        Long newmillisecond = millisecond - (minusMinutes * 1000);
        Date newDate = new Date(newmillisecond);
     ...
     *
     * @param mins 增加或减少分钟数
     * @return 返回字符串类型的世界标准时间, 例如：dateTimeStr为 2014-10-29 11:04:30,mins为 2,则返回 2014-10-29 11:06:30
     */
    public String getDateTimeAddMin(int mins) {
        ...
        cal.add(Calendar.MILLISECOND, 1000 * 60 * mins);
    ...
    /**
     * number表示几个月    比如上一个月是-1    上上个月是-2
     * 获得指定时间的上一个月的时间,并以yyyy-MM-dd HH:mm:ss的格式返回字符串类型时间
     *
     * @return 返回字符串类型时间, 例如：当前时间为2014-10-29 11:04:30,则返回 2014-09-29 11:04:30
     */
    public String getLastMonthDateTime(int number, String dateTimeStr) {
        return addMonthDateTime(number, dateTimeStr, dateTimeFormat);
    }

    /**
     * number表示几个月    比如上一个月是-1    上上个月是-2
     * 获得指定时间的上一个月的时间,并以yyyy-MM-dd 的格式返回字符串类型时间
     *
     * @return 返回字符串类型时间, 例如：当前时间为2014-10-29 ,则返回 2014-09-29
     */
    public String addMonthDate(int number, String dateTimeStr) {
        return addMonthDateTime(number, dateTimeStr, dateFormat);
    }

    public String addMonthDateTime(int number, String dateTimeStr, String format) {
        SimpleDateFormat df = new SimpleDateFormat(format);
        Date date = parseDate(dateTimeStr, format);
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
        cal.add(Calendar.MONTH, number);
        Date dd = cal.getTime();
     ...
     * 把时间段拆分为日期分钟集合
     * <p>
     * 返回日期集合对, 例如输入2014-06-01 11:00:00, 2016-06-01 11:10:00, 输出 {2014-06-01 11:00:00 : 2014-06-01 11:10:00,... 2014-06-01 13:20:00:2014-06-01 13:30:00}
     */
     ...
     */
    public LinkedHashMap<String, String> splitDateRangeToMinute(String startDate, String endDate, int minute) {
        LinkedHashMap<String, String> hourMap = new LinkedHashMap<String, String>();
        Calendar calDate = Calendar.getInstance();
        do {
            if (null != dateTimeFormat) {
            ...
            calDate.add(Calendar.MINUTE, +minute);
            SimpleDateFormat sdf = new SimpleDateFormat(dateTimeFormat);
            String oneMinuteDate = sdf.format(calDate.getTime());
            if (oneMinuteDate.compareTo(endDate) > 0) {
                oneMinuteDate = endDate;
            }
            hourMap.put(startDate, oneMinuteDate);
            startDate = oneMinuteDate;
        } while (startDate.compareTo(endDate) < 0);
        return hourMap;
    }
     ...
     * @param dateformate yyyy-mm-dd
     * @return
     */
    public Integer getMonthDays(String time, String dateformate) {
        //获取当前时间
        Calendar calendar = Calendar.getInstance();
        Date date = this.getDateD(0, dateformate, time);
        calendar.setTime(date);
     ...
     * @return
     */
    public String formateTime(String timeStr, String beforFormate,
                              String afterFormate) {
        String formateTime = null;
        try {
            SimpleDateFormat in = new SimpleDateFormat(beforFormate);
            SimpleDateFormat out = new SimpleDateFormat(afterFormate);
            formateTime = out.format(in.parse(timeStr.replace(".", "/")));
        } catch (ParseException e) {
    ...
    public String formatDateTime(Date dateTime) {
        ...
        SimpleDateFormat format = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss zzz", Locale.CANADA);
     ...
     * 把时间段拆分为日期集合对
     * <p>
     * 返回日期集合对, 例如输入2014-06-01 11:00:00, 2016-06-10  11:00:00, 输出 {2014-06-01 11:00:00 : 2014-06-02 11:00:00,... 2014-06-09 11:00:00:2014-06-10 11:00:00}
     */
    public LinkedHashMap<String, String> splitDateRangeToDay(String startDate, String endDate, int day) {
        LinkedHashMap<String, String> hourMap = new LinkedHashMap<String, String>();
        Calendar calDate = Calendar.getInstance();
        do {
            if (null != dateTimeFormat) {
    ...
    public String getCurrentDateTimeForTimeZone(String zoneId) {
        SimpleDateFormat sdf = new SimpleDateFormat(dateTimeFormat);
        int diffTime = TimeZone.getDefault().getRawOffset()
                - TimeZone.getTimeZone(zoneId).getRawOffset();
    ...
    public String getDateTime(int count, String dateStr, String zoneId) {
        Calendar calDate = Calendar.getInstance();
        if (null != dateStr) {
            calDate.setTime(parseDate(dateStr, dateTimeFormat));
        }
    ...
    public String covertTimeStamp2TimeString(Long timestamp) {
        SimpleDateFormat sdf = new SimpleDateFormat(dateTimeFormat);
        String sd = sdf.format(new Date(timestamp));
        return sd;
    }

    public Date parseDateTimeByZone(String dateTimeStr, String zoneStr) {
